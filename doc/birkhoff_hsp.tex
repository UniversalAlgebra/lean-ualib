...
\begin{lemma}[\protect{\cite[Ex.~1.26.8]{MR2839398}}]
  \label{ex:1.26.8}
  If $f \in \Hom{\alg{A},\alg{B}}$, $g \in \Hom{\alg{A},\alg{C}}$, 
  $g$ is surjective, and $\ker g \subseteq \ker f$, then $\exists h \in \Hom{\alg{C},\alg{B}}$, $f = h \compose g$.
...
$y_i$, $\dots$, $y_n \in Y$ such that $u = p^{\alg{T}(Y)}(y_1,\dots, y_n)$ and 
